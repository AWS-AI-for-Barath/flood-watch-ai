...
// Initialize S3 Client carefully to catch credential/region errors in Amplify SSR
...
export async function POST(req: Request) {
    if (!s3Client) {
        return NextResponse.json({ error: "S3 Client failed to initialize on the server." }, { status: 500 });
    }
    try {
        let formData;
        try {

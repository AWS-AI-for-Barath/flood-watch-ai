...
export const handler = async (event) => {
    ...
    try {
        ...
        // Step 1: Frame Extraction (with mock fallback)
        let frames = [];
        try {
            frames = await frameExtraction(bucket, key);
        } catch (e) {
            console.log(`[processFloodAI] Skipping Bedrock, generating mock analysis. Extraction error: ${e.message}`);
        }
        console.log(`[processFloodAI] Extracted ${frames.length} frame(s)`);

        // Step 2: Invoke Bedrock for each frame (or use mock)
        const frameResults = [];
        if (frames.length === 0) {
            frameResults.push({
                people_trapped: false,
        ...
        // Step 4: Write to S3 analysis/ prefix (with exact `{}.json` suffix match for the S3 trigger)
